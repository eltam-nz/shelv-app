 ...
 * marks that matter.
 *
 * **Every mark is the blocked colour, including `Disconnected`**, which is
 * grey in its pill. The two places are answering different questions. The
 * drives pane is an inventory: a drive that is not plugged in right now is
 * a neutral fact about that drive. The rule table is answering "can this
 * back up?", and there an unplugged destination is the rule not running —
 * the same outcome as a refused or unidentifiable one, and it should not
 * recede into the grey used for the em dashes and the italic paths around
 * it.
 *
 * Severity is still legible: the glyphs differ, and the hover text says
 * whether plugging the drive in would fix it. Amber against rose at eleven
 * pixels was never carrying that distinction anyway.
 */
